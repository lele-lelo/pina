...
export async function searchIgdbGames(query: string): Promise<TIgdbGame[]> {
  ...
  const acceptedGameTypes = ["Main Game", "Remake", "Remaster"];
  ...
  const fields = `
    fields name, summary, first_release_date, cover.image_id, genres.name, game_type.type, platforms.name, alternative_names.name;
  `;
  const where = `
